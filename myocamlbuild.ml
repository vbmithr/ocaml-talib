(* OASIS_START *)
(* DO NOT EDIT (digest: 6a3a3d62a896a7bece177f20e6256d4c) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("talib", ["lib"])];
     lib_c = [("talib", "lib", [])];
     flags =
       [
          (["oasis_library_talib_cclib"; "link"],
            [(OASISExpr.EBool true, S [A "-cclib"; A "-lta_lib"])]);
          (["oasis_library_talib_cclib"; "ocamlmklib"; "c"],
            [(OASISExpr.EBool true, S [A "-lta_lib"])])
       ];
     includes = [("test", ["lib"])];
     }
  ;;
